...
public class InsertSelectiveMethodGenerator extends AbstractDAOElementGenerator
{
	...
	public void addImplementationElements(TopLevelClass topLevelClass)
	{
		...
		sb.append(daoTemplate.getInsertMethod(
				introspectedTable.getIbatis2SqlMapNamespace(),
				introspectedTable.getInsertSelectiveStatementId(),
				StringUtils.uncapitalize(introspectedTable.getRules()
						.calculateAllFieldsClass().getShortName()))); //$NON-NLS-1$
		...
		if (ibatorContext.getPlugins().daoInsertSelectiveMethodGenerated(
	...
	public void addInterfaceElements(Interface interfaze)
	{
		Set<FullyQualifiedJavaType> importedTypes = new TreeSet<FullyQualifiedJavaType>();
		Method method = getMethodShell(importedTypes);
		addMethodComment(method, "save pojo");
		if (ibatorContext.getPlugins().daoInsertSelectiveMethodGenerated(
	...
	private Method getMethodShell(Set<FullyQualifiedJavaType> importedTypes)
	{
		...
		method.addParameter(new Parameter(parameterType, StringUtils
				.uncapitalize(parameterType.getShortName()))); //$NON-NLS-1$
